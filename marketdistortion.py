...
class GPT2Agent(Agent):
    def __init__(self, unique_id, model, agent_type, memory_size=5):
        ...
        self.x = random.uniform(0, 5) if agent_type == 'buyer' else random.uniform(5, 10)  # 에이전트의 x 좌표 (랜덤 초기화)
        self.y = random.uniform(0, 5) if agent_type == 'buyer' else random.uniform(5, 10)  # 에이전트의 y 좌표 (랜덤 초기화)
        ...
        for buyer in buyers:
            for seller in sellers:
                if not buyer.match and not seller.match and buyer.price >= seller.price:
                    ...
                    print(
                        f"Transaction: Buyer at {buyer.price:.2f} - Seller at {seller.price:.2f} - Price: {transaction_price:.2f}")
